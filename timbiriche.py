...
# --- CSS DEFINITIVO PARA TABLA FIJA ---
st.markdown("""
    <style>
    .timbiriche-table { border-collapse: collapse; margin: auto; }
    .punto { font-size: 24px; color: white; width: 30px; height: 30px; text-align: center; }
    ...
    .linea-v-llena { border-left: 6px solid #AAAAAA; height: 60px; margin-left: 12px; }
    
    /* Cuadros llenos */
    .cuadro-tutu { background-color: #005A87; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold; border-radius: 4px; }
    .cuadro-abuelita { background-color: #7A2E16; width: 60px; height: 60px; display: flex; align-items: center; justify-content: center; font-weight: bold; border-radius: 4px; }
    </style>
...
# Construcción manual del tablero para evitar decalajes
for r in range(5):
    # Fila horizontal
    row_cols = st.columns([1, 2, 1, 2, 1, 2, 1, 2, 1])
    for c in range(4):
    ...
    if r < 4:
        row_cols_v = st.columns([1, 2, 1, 2, 1, 2, 1, 2, 1])
        for c in range(5):
