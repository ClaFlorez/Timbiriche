...
/* ── Responsive: forzar fila única en cualquier pantalla ── */
...
.linea-h-tutu::before {{
    ...
    box-shadow:0 0 14px {COLOR_TUTU}; }}
.linea-h-abuelita::before {{
    content:""; display:block; width:100%;
    border-top:8px solid {COLOR_ABUELITA}; border-radius:10px;
    box-shadow:0 0 14px {COLOR_ABUELITA}; }}

/* ── Líneas verticales ── */
.linea-v-tutu, .linea-v-abuelita {{
    display:flex; justify-content:center; height:45px; width:100%; }}
.linea-v-tutu::before {{
    content:""; display:block; height:100%;
    border-left:8px solid {COLOR_TUTU}; border-radius:10px;
    box-shadow:0 0 14px {COLOR_TUTU}; }}
.linea-v-abuelita::before {{
    content:""; display:block; height:100%;
    border-left:8px solid {COLOR_ABUELITA}; border-radius:10px;
    box-shadow:0 0 14px {COLOR_ABUELITA}; }}
...
h1 {{ color:white !important; text-align:center;
    font-size:1.8rem !important; margin-bottom:2px !important; }}
...
@keyframes volar {{
    ...
    60%  {{ opacity:1; }}
    100% {{ transform:translate(calc(-50% + var(--tw)), calc(-50% + var(--th)))
            scale(1.4) rotate(var(--tr)); opacity:0; }}
}}
</style>
            ...
            if c < COLS:
                if (r, c) in juego["cuadros"]:
                    ...
                    cols_v[c*2+1].markdown(
                        f"<div class='cuadro-{{color}}'>{{own[0]}}</div>",
                        unsafe_allow_html=True)
                else:
                    cols_v[c*2+1].markdown(
                        "<div style='height:45px;'></div>", unsafe_allow_html=True)
...
if fin:
    ...
    # Efectos: solo la primera vez en esta sesión
    if not st.session_state.fin_visto:
        ...
        for _ in range(80):
            ...
            emoji = random.choice(["⭐", "🌟", "✨", "💫"])
            fx += (f'<div class="star-fx" style="--tw:{{tw}}px;--th:{{th}}px;--tr:{{tr}}deg;'
    ...
    <div style="text-align:center; padding:28px 20px 20px;
      ...
      <div class="marcador-final">
        {{EMOJIS['Tutu']}} Tutu
        <span style="font-size:1.8rem;color:white;">{{pt}}</span>
        &nbsp;–&nbsp;
        <span style="font-size:1.8rem;color:white;">{{pa}}</span>
